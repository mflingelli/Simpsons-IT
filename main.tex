\author{\bf Dipl.-Inform. (univ.) Markus M. Flingelli}
\begin{document}
...
\noindent\Hinweis[0.975\textwidth]{t}{Anmerkung}{%
\begin{itemize}
	\item Datei mit der ersten Million Nachkommastellen von $\pi$: \href{http://www.aip.de/~wasi/PI/Pibel/pibel_10mio.pdf}{\url{http://www.aip.de/~wasi/PI/Pibel/pibel\_10mio.pdf}}
	\item Der Rekord im Memorieren lag zum Zeitpunkt der Entstehung der Folge bei ca. 40.000 Nachkommastellen.
	\item Den aktuellen Weltrekord im Memorieren der Zahl $\pi$ hält der Inder Suresh Kumar Sharma mit aufgesagten 70.030 Nachkommastellen. Aufgestellt wurde der Rekord am 21 Oktober 2015. Die Vortragsdauer lag bei gut 17 Stunden (siehe \href{https://www.pi-world-ranking-list.com/?page=lists&category=pi}{\url{https://www.pi-world-ranking-list.com/?page=lists&category=pi}})
	\item  Die Pointe mit der tatsächlichen 40.000. Stelle von $\pi$ war jedoch eine Herausforderung für die Drehbuchautoren, da sie 1993 nicht einfach die bekannten Stellen googeln konnten. Der NASA-Mathematiker David Bailey half aus, indem er eine gedruckte Kopie der ersten 40.000 Nachkommastellen von $\pi$ schickte, um zu bestätigen, dass die letzte Ziffer tatsächlich 1 ist.
\end{itemize}
...
\begin{itemize}
	...
	\item Registernummer der Nimbus, des Raumschiffes von 25-Sterne General Zapp Brannigan ist 1729
\end{itemize}
\end{block}
}
...
\begin{minipage}{0.6\textwidth}
	\begin{align}
		\sqrt{a} + \sqrt{a} &= \sqrt{c} \\
		2\sqrt{a}           &= \sqrt{c} && \mid (\dots)^2 \\
		4a                  &= c
	\end{align}
\end{minipage}
\end{block}
...
Da $a > 0$ ist, gilt stets $4a > a$. Da $c = 4a$ ist, folgt daraus:
\begin{equation}
	b = 4a > a \implies c > a \implies a \neq c
\end{equation}

Für jedes gültige Dreieck mit den Seitenlängen $a$, $a$ und $c$ muss nach der \textbf{Dreiecksungleichung} gelten, dass die Summe zweier Seiten streng größer ist als die dritte Seite:
\begin{equation}
	a + a > c \iff 2a > c
\end{equation}

Da aus Homers Aussage jedoch $c = 4a$ folgt, ergibt sich durch Einsetzen:
\begin{align}
	2a &> 4a && \mid \, : a \quad (\text{da } a > 0) \\
	2 &> 4 \quad \text{\textbf{(Widerspruch!)}}
\end{align}

\textbf{Fazit:} Die Bedingung $c = 4a$ verletzt die Dreiecksungleichung. Es kann somit kein geschlossenes Dreieck existieren, das diese Gleichung erfüllt.
}
}

\subsection{Mengenlehre}

\subsubsection{Aleph-Funktion}
\frame{
\only<presentation>{\frametitle{Aleph-Funktion}}
\begin{block}{Definition:}
\begin{itemize}
...
\frame{
\only<presentation>{\frametitle{Mächtigkeit}}
\begin{block}{}
\begin{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Hotel ist mit unendlich vielen Gästen belegt, trotzdem können abzählbar unendlich viele Neuankömmlinge aufgenommen werden: Alle Zimmer mit geraden Zimmernummern werden freigemacht und neue Gäste werden in diesen untergebracht.
	\end{itemize}
\end{itemize}
}
}
...
\subsubsection{Mooresches Gesetz}
...
\begin{frame}[fragile]
\only<presentation>{\frametitle{ASCII}}
\begin{block}{Die Panik-Amok-Horror-Show}
\begin{itemize}
	\item Folgende hexadezimal codierte Zeichenkette ist zu sehen:
	\begin{lstlisting}[style=base]
	46 72 69 6E 6B 20 72 75 6C 65 73 21
	\end{lstlisting}
	\item Als ASCII-Code interpretiert bedeutet sie:
	\begin{lstlisting}[style=base]
	Frink rules!
	\end{lstlisting}
\end{itemize}
...
\begin{frame}[fragile]
\only<presentation>{\frametitle{BASIC}}
...
\begin{block}{Mensch gegen Maschine}
\begin{itemize}
	\item In Springfield entsteht eine Schule mit Schwerpunkt auf den MINT-Fächern
	\item Der \glqq Algorithmus\grqq\ bereitet Schüler auf zukünftige Aufgaben vor und empfiehlt den hochbegabten Schülern \glqq Programming with C+++\grqq
\end{itemize}
\end{block}
}
...
\frame{
\only<presentation>{
	\frametitle{Turing-Test}
}
\begin{block}{Im Körper des Freundes}
\begin{itemize}
  \item Bender möchte mit robo-ungarischem Kaiser Nikolai den Körper tauschen
  \item Bender steckt im Körper von Amy
  \item Kaiser Nikolai will mit Hilfe des umgekehrten Turing-Tests prüfen, ob Bender auch wirklich ein Roboter ist
\end{itemize}
\end{block}
}
...
\begin{block}{}
\begin{itemize}
  \item 1950 von Alan Turing formulierte Idee, um festzustellen, ob eine Maschine ein dem Menschen gleichwertiges Denkvermögen hätte
  \item Ursprünglich von Turing als \glqq imitation game\grqq\ bezeichnet
  \item CAPTCHA-Verfahren leitet Namen hiervon ab: Completely Automated Public Turing test to tell Computers and Humans Apart
\end{itemize}
\end{block}
}
